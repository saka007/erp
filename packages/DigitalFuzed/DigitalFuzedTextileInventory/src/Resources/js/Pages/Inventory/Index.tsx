        ...
        <AuthenticatedLayout breadcrumbs={[{ label: t('Textile') }, { label: t('Inventory') }]} pageTitle={t('Textile Inventory')}>
            ...
            >
                        ...
                        <Card>
                            <CardContent className="p-5">
                                ...
                                <form onSubmit={submitMovement} className="space-y-4">
                                    <SelectField label={t('Movement Type')} value={movementForm.data.movement_type} onChange={(value) => movementForm.setData('movement_type', value)} options={movementTypes} required />
                                    ...
                                    <div className="grid grid-cols-2 gap-3">
                                        <SelectField label={t('From Location')} value={movementForm.data.location_from} onChange={(value) => movementForm.setData('location_from', value)} options={locations.map((location) => location.name)} includeEmpty emptyLabel={t('Select location')} />
                                        <SelectField label={t('To Location')} value={movementForm.data.location_to} onChange={(value) => movementForm.setData('location_to', value)} options={locations.map((location) => location.name)} includeEmpty emptyLabel={t('Select location')} />
                                    </div>
                                    <div className="grid grid-cols-2 gap-3">
                                    ...
                                    <SelectField label={t('Status')} value={movementForm.data.status} onChange={(value) => movementForm.setData('status', value)} options={movementStatuses} required />
                ...
                <TabsContent value="controls" className="space-y-6">
                    <Card>
                        <CardContent className="p-5">
                            <div className="mb-5 flex items-center gap-2">
                                <Boxes className="h-5 w-5 text-violet-600" />
                                <h2 className="font-semibold">{t('Location and Lot Controls')}</h2>
                            </div>
                            <form onSubmit={submitLocation} className="grid gap-4 xl:grid-cols-4">
                                ...
                                <SelectField label={t('Location Type')} value={locationForm.data.location_type} onChange={(value) => locationForm.setData('location_type', value)} options={['warehouse', 'loom-floor', 'process-house', 'dispatch', 'quarantine']} required />
                            ...
                            <form onSubmit={submitLocationArchive} className="mt-4 grid gap-4 xl:grid-cols-4">
                                <SelectField label={t('Archive Location')} value={locationArchiveForm.data.location_id} onChange={(value) => locationArchiveForm.setData('location_id', value)} options={locations.map((location) => String(location.id))} includeEmpty emptyLabel={t('Select active location ID')} required />
                            ...
                            <form onSubmit={submitLotUpdate} className="mt-4 grid gap-4 xl:grid-cols-4">
                                <SelectField label={t('Update Lot Status')} value={lotUpdateForm.data.lot_id} onChange={(value) => lotUpdateForm.setData('lot_id', value)} options={lots.filter((lot) => lot.is_active !== false).map((lot) => String(lot.id))} includeEmpty emptyLabel={t('Select active lot ID')} required />
                                <SelectField label={t('New Status')} value={lotUpdateForm.data.status} onChange={(value) => lotUpdateForm.setData('status', value)} options={['active', 'hold', 'inactive']} required />
                            ...
                            <form onSubmit={submitLotArchive} className="mt-4 grid gap-4 xl:grid-cols-4">
                                <SelectField label={t('Archive Lot')} value={lotArchiveForm.data.lot_id} onChange={(value) => lotArchiveForm.setData('lot_id', value)} options={lots.filter((lot) => lot.is_active !== false).map((lot) => String(lot.id))} includeEmpty emptyLabel={t('Select active lot ID')} required />
                    ...
                    <Card>
                        <CardContent className="p-5">
                            <div className="mb-5 flex items-center gap-2">
                                <Boxes className="h-5 w-5 text-violet-600" />
                                <h2 className="font-semibold">{t('Reservation Controls')}</h2>
                            </div>
                            <form onSubmit={submitReservationRelease} className="grid gap-4 xl:grid-cols-4">
                                <SelectField label={t('Reservation ID')} value={reservationReleaseForm.data.reservation_id} onChange={(value) => reservationReleaseForm.setData('reservation_id', value)} options={reservations.map((reservation) => String(reservation.id))} includeEmpty emptyLabel={t('Select active reservation')} required />
                            ...
                            <form onSubmit={submitReservationAllocate} className="mt-4 grid gap-4 xl:grid-cols-4">
                                <SelectField label={t('Allocate Reservation ID')} value={reservationAllocateForm.data.reservation_id} onChange={(value) => reservationAllocateForm.setData('reservation_id', value)} options={reservations.map((reservation) => String(reservation.id))} includeEmpty emptyLabel={t('Select active reservation')} required />
                                <Field label={t('Allocation Reference ID')} type="number" value={reservationAllocateForm.data.allocation_reference_id} onChange={(value) => reservationAllocateForm.setData('allocation_reference_id', value)} required />
                                <Field label={t('Allocation Reference Type')} value={reservationAllocateForm.data.allocation_reference_type} onChange={(value) => reservationAllocateForm.setData('allocation_reference_type', value)} />
                ...
                <TabsContent value="records" className="space-y-6">
                    <Card>
                        <CardContent className="p-5">
                            ...
                            <form onSubmit={submitMovementFilter} className="grid gap-4 xl:grid-cols-5">
                                <SelectField label={t('Type')} value={movementFilterForm.data.movement_type} onChange={(value) => movementFilterForm.setData('movement_type', value)} options={movementTypes} includeEmpty emptyLabel={t('All types')} />
                                <SelectField label={t('Status')} value={movementFilterForm.data.status} onChange={(value) => movementFilterForm.setData('status', value)} options={movementStatuses} includeEmpty emptyLabel={t('All statuses')} />
                                ...
                                <SelectField label={t('Location')} value={movementFilterForm.data.location} onChange={(value) => movementFilterForm.setData('location', value)} options={locations.map((location) => location.name)} includeEmpty emptyLabel={t('All locations')} />
                    ...
                    <div className="grid gap-6 xl:grid-cols-2">
                        <Card><CardContent className="p-0"><DataTable data={locations} columns={[{ key: 'name', header: t('Location') }, { key: 'code', header: t('Code'), render: optional }, { key: 'location_type', header: t('Type') }]} emptyState={<NoRecordsFound icon={Boxes} title={t('No textile locations found')} description={t('Create dedicated locations to control stock movement flow.')} />} /></CardContent></Card>
                        <Card><CardContent className="p-0"><DataTable data={lots} columns={[{ key: 'lot_reference', header: t('Lot Reference') }, { key: 'received_quantity', header: t('Received') }, { key: 'available_quantity', header: t('Available') }, { key: 'reserved_quantity', header: t('Reserved') }, { key: 'status', header: t('Status') }, { key: 'id', header: t('Details'), render: (value: number) => (<Button type="button" variant="ghost" size="sm" onClick={() => router.visit(route('textile.inventory.lots.show', value))}>{t('View')}</Button>) }]} emptyState={<NoRecordsFound icon={Boxes} title={t('No textile lots found')} description={t('Create the first lot to begin tracking stock by lot.')} />} /></CardContent></Card>
                        <Card><CardContent className="p-0"><DataTable data={movements} columns={[{ key: 'movement_type', header: t('Type') }, { key: 'lot_reference', header: t('Lot'), render: optional }, { key: 'location_from', header: t('From'), render: optional }, { key: 'location_to', header: t('To'), render: optional }, { key: 'quantity', header: t('Qty') }, { key: 'status', header: t('Status') }]} emptyState={<NoRecordsFound icon={MoveRight} title={t('No textile movements found')} description={t('Record the first inventory movement to build stock history.')} />} /></CardContent></Card>
                        <Card><CardContent className="p-0"><DataTable data={reservations} columns={[{ key: 'lot_reference', header: t('Lot') }, { key: 'reserved_quantity', header: t('Reserved Qty') }, { key: 'reference_type', header: t('Reference Type'), render: optional }, { key: 'reference_id', header: t('Reference ID'), render: optionalNumber }, { key: 'status', header: t('Status') }]} emptyState={<NoRecordsFound icon={Boxes} title={t('No textile reservations found')} description={t('Reserve stock to control lot availability by demand.')} />} /></CardContent></Card>
                    </div>
                </TabsContent>
            </Tabs>
        </AuthenticatedLayout>
