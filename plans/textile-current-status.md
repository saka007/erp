...
## Staff always branch-scoped — warehouse/branch pickers + mandatory assignment (deployed 2026-08-10, commit b4946fb6e, run 31345647720)

- **Problem reported**: on the Textile Sales form the "Delivery Warehouse" dropdown listed warehouses from BOTH branches for a manager (staff user John Smith, single branch assignment). The warehouse picker read `session('active_branch_id')` directly, and for assigned users the middleware resolved the branch but never persisted it to the session → session null → all tenant warehouses shown.
- **Fix**:
  - `TextileSalesController::warehouseOptions()` now resolves the branch via `TextileBranchScope::branchIdForCreate()` (assignments first, then session/manage-all, then employee record) instead of raw session — single-branch staff only ever see their own branch's warehouses.
  - `HandleInertiaRequests` now persists the resolved active branch to the session for assigned users (single → that branch; multiple → session choice or first), so all session-based readers stay scoped consistently. The header branch dropdown is no longer granted to staff via `manage-any-branches`: only tenant roots (company/superadmin) or staff with multiple assigned branches get the switcher, limited to assigned branches.
  - `BranchContextController::update` allows staff to switch only within multiple assigned branches (the permission no longer grants switching for staff).
  - `StoreUserRequest`/`UpdateUserRequest`: `branch_ids` is now `required|array|min:1` for staff users when the tenant has branches (superadmin-created company accounts are tenant roots and exempt). User create/edit forms mark Branches as required with a client-side guard so an admin can no longer create/leave a staff account without a branch assignment.
- **Verified on production**: logged in as `john.smith@company.com` (staff, assignment `[1]`): header has NO branch dropdown; Textile Sales form Delivery Warehouse shows only `Main WH` (was `Main WH` + `Branch B WH`). `php -l` clean, `npx tsc --noEmit` 0 errors, `npm run build` pass, 84 textile feature tests pass (1449 assertions). The 12 failing tests in the full suite are pre-existing stock Laravel Auth/Profile tests, confirmed by stashing (they fail without these changes too).

## Sales form warehouse selection removed — auto-derived from active branch (deployed 2026-08-10, commit 73c35f6cb, run 31346347785)

- **Request**: remove the warehouse selection from the sales order form entirely — the warehouse is globally derived from the user's active branch (same behavior as the Quotation module's auto-selected warehouse).
- **Change**: `Delivery Warehouse` dropdown removed from the Sales Order form (also removed the `warehouse` form field and the `warehouseOptions` prop). Backend `storeSalesOrder()` now auto-derives the warehouse name via new `TextileSalesController::activeBranchWarehouseName()` — branch resolved through `TextileBranchScope::branchIdForCreate()` (assignments first), then the first warehouse in that branch scope is stored in `metadata['warehouse']`. `warehouseOptions()` method removed.
- **Verified on production**: logged in as `john.smith@company.com` — the Sales Order form no longer shows "Delivery Warehouse" (`hasDeliveryWarehouse: false`); selects are only section/customer/takha-lot. `grep -c 'Delivery Warehouse'` on the deployed Sales/Index.tsx = 0; `grep -c activeBranchWarehouseName` on deployed controller = 2. `php -l`, `npx tsc --noEmit`, `npm run build`, 84 textile tests all pass.

## Quotation items — duplicate takha lot selection prevented (deployed 2026-08-10, commit 2627d2bcd, run 31371906730)

- **Problem reported**: on the Quotation items table ("Add Item" rows) the same takha lot could be picked in more than one row, which would corrupt the eventual sales order conversion.
- **Frontend fix** (`QuotationItemsTable.tsx` + `ProductSelector.tsx`): each item row now computes the set of `lot_reference`s already chosen in the other rows and passes it to `ProductSelector` as `disabledLotReferences`. Lot options already selected in another row render disabled (greyed out via Radix `data-[disabled]`), while the current row's own lot stays selectable. Pattern mirrors the existing Sales order form `selectedLots` logic.
- **Backend fix** (`StoreQuotationRequest` + `UpdateQuotationRequest`): new `withValidator()` runs an `after` callback that rejects any item with a `lot_reference` already present on a previous item (`This lot has already been added to the quotation. Each lot can only be added once.`). Only non-null `lot_reference` values are compared, so catalog product rows (multiple nulls) are unaffected.
- **New test**: `tests/Feature/QuotationDuplicateLotValidationTest.php` — 4 cases: duplicate lot rejected on store, unique lots pass, product rows with null `lot_reference` pass, duplicate rejected on update. `php artisan test --filter=QuotationDuplicateLotValidationTest` → 4 passed (6 assertions).
- **Verification**: `php -l` clean on both request files, `npx tsc --noEmit` 0 errors, `npm run build` pass, full suite 111 passed (12 pre-existing stock Laravel Auth/Profile failures unchanged).

## Per-party credit system — credit days + credit_enabled for purchase & sales (deployed 2026-08-10)

- **Feature**: a configurable credit system for both purchase (vendors) and sales (customers), set **per party** on the vendor/customer master forms and on the Payments page. Each party can opt into credit (`credit_enabled`) and specify a payment window (`credit_days`, e.g. Net 30), plus an optional `credit_limit`.
- **Data model**: new migration `2026_08_10_000001_add_credit_enabled_to_vendors_customers_table.php` adds `credit_enabled` (boolean, default false) to both `vendors` and `customers`. `Customer` + `Vendor` models `$fillable` now include `credit_days`, `credit_enabled`, `reminder_enabled`, `branch_id` (they previously omitted these, so the Payments-page credit settings were silently dropped) with boolean casts for `credit_enabled`/`reminder_enabled`.
- **Master forms** (Customer + Vendor Create/Edit): new `Credit Days` number input and `Credit Enabled` checkbox beside the existing Credit Limit + Credit Currency fields. Backend `CustomerController`/`VendorController` store/update persist `credit_days` + `credit_enabled` (vendors also `credit_limit`, which was previously never persisted); request validation added in all 4 request files (`credit_days` integer 0–3650, `credit_enabled` boolean, `credit_limit` numeric ≥ 0).
- **Payments page** (`Payments/Index.tsx` + `TextilePaymentController`): the Credit & Reminder Settings form now has a `credit_enabled` toggle; party rows in the summary table show a Credit On/Off badge. `TextilePaymentController@updateCredit` requires `credit_enabled`; `TextilePaymentReminderService@updateCredit` persists it, `creditDaysFor()` returns 0 for parties that did not opt in, and party master/vendor-row payloads include `credit_enabled`.
- **GRN → purchase invoice** (`TextileProcurementService@createPurchaseInvoiceFromGrn`): replaces the hardcoded `due_date = now()+15` / null payment terms. Now resolves the vendor record (by `user_id` or `id`, tenant-scoped) and, when the vendor has `credit_enabled` + `credit_days > 0`, sets `due_date = invoice_date + credit_days` and `payment_terms = "Net N"`. Otherwise the invoice is due on the invoice date (pay on delivery, `payment_terms = null`).
- **Document credit indicators**: Sales order form shows a green "Credit Terms: payment allowed within N days" banner when the selected customer has credit enabled (`TextileSalesController@customerOptions` now ships `credit_days`/`credit_enabled`); Quotation Create/Edit show the same hint below the customer picker (`QuotationController@customersForSelect`); Procurement requisition form + supplier info panel show vendor credit terms (`TextileProcurementController@supplierSummaries` + `textile-info-panel.tsx`).
- **New test**: `tests/Feature/Textile/TextileCreditSystemTest.php` — 4 cases: GRN invoice due date honors vendor credit days when credit enabled (asserts due_date = today+30 and `Net 30`), falls back to same-day due + null payment terms when credit disabled, customer master store persists credit fields, vendor master store persists credit_limit/credit_days/credit_enabled. `php artisan test tests/Feature/Textile/TextileCreditSystemTest.php` → 4 passed (18 assertions); full textile suite 87 passed (1447 assertions), `npx tsc --noEmit` 0 errors, `npm run build` pass.

## Parties page REMOVED + credit/reminder consolidated into master forms (2026-08-12, commit pending, GH run pending)

- **Request**: after adding the SEO `/vendors` Vendors Management page, the old unified Parties page (`/textile/parties`) became redundant — its only unique value was the Credit & Reminder Settings card, and once credit/reminder settings moved into the Vendor/Customer master forms it was a read-only directory duplicating the masters.
- **Consolidation**: `reminder_enabled` checkbox added to Vendor + Customer Create/Edit forms (beside existing Credit Limit / Credit Days / Credit Enabled); `Store/UpdateVendorRequest` + `Store/UpdateCustomerRequest` accept `reminder_enabled`; `VendorController` + `CustomerController` store/update persist it (models already had the column + fillable + boolean cast). Vendor + Customer TS types (`Vendor`, `CreateVendorFormData`, `Customer`, `CustomerFormData`) gained `reminder_enabled`.
- **Removed**: `GET /textile/parties` route, `TextilePartyController`, `Parties/Index.tsx` page, its menu entry (Daily Operations), and `TextilePartyAdminTest.php` (6 tests). `TextilePaymentReminderService` and the `textile.payments.credit.update` route are UNCHANGED — still used by the Payments workflow page + reminder console command. `TextilePartyBranchService`/`TextilePartyBranchController` (branch assignment) are a separate feature and untouched.
- **Verified**: `php -l` clean on all 7 PHP files; `npx tsc --noEmit` 0 errors; `npm run build` green; `php artisan test tests/Feature/Textile` → 108 passed (1679 assertions); `php artisan test tests/Feature/Account` → 9 passed (127 assertions).

## Unified Parties page — yarn/sizing/powerloom vendors + customers in one place with credit settings (deployed 2026-08-10, commit 56bdbe3b4, run 31403168629)

- **Request**: party management was confusing because yarn vendors (buy-from), sizing vendors (sell-yarn-to for beaming), powerloom vendors and customers (buyers of takhas) lived in separate places with no single view. Wanted one URL with filters + the full credit system on every party type.
- **New page** `GET /textile/parties` (`textile.parties.index`, `TextilePartyController@index`): lists **all** parties (vendors of every supplier_type + customers) in one unified table with:
  - **Category filter** (All Parties / Yarn Suppliers / Sizing Vendors / Powerloom Vendors / Customers (Buyers) / Other Vendors) — mapped from `supplier_type` for vendors, `party_type=buyer` for customers.
  - **Branch filter** + **search** (party name or party code).
  - KPI cards (Parties / Yarn Suppliers / Sizing Vendors / Customers / Credit Enabled).
  - Per-row Credit On/Off badge, Credit Days, Credit Limit (INR), Reminders, Branch, and **"Open Master"** action that jumps to `account.vendors.index?supplier_type=` or `account.customers.index` (gated by edit-vendors/edit-customers permissions).
  - **Credit & Reminder Settings** form on the page (party select, credit days/limit, credit_enabled, branch, reminder_enabled) posting to the existing `textile.payments.credit.update` endpoint — same credit system for every party type.
- **Backend**: `TextilePaymentReminderService@partyMasters()`/`partyMaster()` now ship `supplier_type`, `party_code` (vendor_code/customer_code) and `branch_name` per party. New `TextilePartyController` filters by category/branch/search with tenant scoping (via `partyMasters` tenant filter), gated by textile access (company/superadmin/staff) + `payments` capability.
- **Menu**: new **Parties** entry as first child of Textile > Daily Operations (above Procurement), capability `payments`, `Users` icon — `textileCoreCompanyMenu` in `company-menu.ts`.
- **Fix — sizing vendor dropdown not filtering** (`Account Vendors/Index.tsx`): the Supplier Type select only updated local filter state; it now also fires `router.get(route('account.vendors.index'), ...)` on change so the list + URL actually filter (yarn/sizing/etc).
- **Fix — Payments page credit save silently failing** (`Payments/Index.tsx`): the Credit & Reminder Settings form posted `party_key` (e.g. `supplier:3`) while the backend `updateCredit` validates required `party_type` + `party_id` — every save from the UI failed validation, which is why customer credit seemed ignored. Now uses `creditForm.transform()` to split `party_key` into `party_type`/`party_id` before posting. Same transform added to the new Parties page.
- **Fix — branch column bug** (`TextilePaymentReminderService@branchOptions`/`branchName`): queried `name` column on `branches` table which only has `branch_name` — any page rendering branch options (Payments reminders) would 500 when a branch existed. Both methods now read `branch_name`.
- **New test**: `tests/Feature/Textile/TextilePartyAdminTest.php` — 5 cases (114 assertions): lists all party types, filters by category (sizing/customer/yarn), filters by search + branch, includes credit fields (credit_enabled/credit_days/credit_limit/reminder_enabled), rejects non-company users (403). `php artisan test tests/Feature/Textile/TextilePartyAdminTest.php` → 5 passed; full textile suite 92 passed (1561 assertions); `npx tsc --noEmit` 0 errors; `npm run build` pass.
- **Verified on production**: deployed chunk `Index-DG17MZYh.js` contains `textile.parties.index`; `Index-Dg3quUx4.js` contains the credit-save transform (`party_type:e,party_id:d`); route registered (`php artisan route:list --name=textile.parties` → `GET|HEAD textile/parties`); controller present; menu `route('textile.parties.index')` in deployed `company-menu.ts`; unauthenticated `curl https://erp.digitalfuzed.com/textile/parties` → 302 (login redirect, no 500).

## Master Setup menu restructure — Source Types/Actions admin-only + grouped sections (2026-08-10)

- **Request**: Source Types and Source Actions generated a lot of unwanted menu items — they were duplicated across 7 setup sections (Manufacturing, Inventory, Sales, Processing, Quality, Packing, Dispatch) = 14 menu entries pointing to the same domain-scoped CRUD pages, and several setup sections existed only to host them. Wanted them visible only to admin users.
- **RBAC gating (`adminOnly`)**: added `adminOnly?: boolean` to `NavItem` (`resources/js/types/index.d.ts`) and a `filterByAdminOnly` step in the menu pipeline (`resources/js/utils/menu.ts`, after capability filtering) — admin = `user.type === 'company' | 'superadmin'` or `roles.includes('superadmin')` (matches the existing tenant-root check in `HandleInertiaRequests`). Items flagged `adminOnly` (and empty parent sections that collapse after filtering) are hidden for staff/manager users.
- **Consolidation**: all 14 Source Types / Source Actions entries removed from the 7 setup sections; ONE `Source Types & Actions` entry (adminOnly, `Workflow` icon) added under Core Config pointing to `textile.master-domains.source-types.index` with domain `manufacturing`.
- **Domain switcher on Masters page**: `Masters/Index.tsx` now shows a Source Types ↔ Source Actions toggle plus a domain chip bar (Global/Manufacturing/Inventory/Procurement/Sales/Processing/Quality/Dispatch/Packing/Transport) for every domain-aware master — `router.get` to `textile.master-domains.{master}.index?domain=` with `preserveState`. All domain-scoped records remain reachable from one entry per master.
- **Section flattening (14 → 6)**: Master Setup regrouped into Core Config (incl. Source Types & Actions), Manufacturing (vendors, machine/shed/loom/breakdown/maintenance/cost/fabric-grade masters), Quality (inspection results, fabric defects), Dispatch & Transport (truck/container/drivers/vehicles/routes/LR/e-way + fuel/freight/transport-maintenance), CRM & Suppliers (customer masters + transport vendors/ratings/performance), Products (variants/images/documents).
- **Helper text + breadcrumb sync**: updated "Master Setup > Loom Setup/Beam and Cost Setup/Packing Setup/Processing Setup/Transport Setup/CRM Setup/Supplier Setup/Product Setup" references across Manufacturing, Packing, Processing, Transport, Account (CustomerCategories/Contacts/Documents/FollowUps/PriceLists/VendorPerformance/VendorRatings) and ProductService pages to the new group names.
- **Verified**: `npx tsc --noEmit` 0 errors; `npm run build` pass; `php artisan test --filter=Textile` 93 passed (1581 assertions).

## Master Setup hidden from non-admin + Parties branch auto-scope (deployed 2026-08-11)

- **Master Setup admin-only (commit `ff3408b54`, run 31417442111)**: per the admin request, the entire **Master Setup** menu parent is now `adminOnly: true` in `company-menu.ts`, and the 10 backend master controllers were tightened to `['company','superadmin']` — non-admin/staff users get no menu entry and HTTP 403 on direct route access. Verified on prod: staff (user 3) → 403 on `/textile/master-setup/manufacturing/source-types`; company (user 2) → 200. 93 textile tests pass (1581 assertions).
- **Parties branch auto-scope (commits `130312b52`, `545d1c735`, `e6bf5af80`, run 31422444816)**: the branch selector was removed from the Parties page filter form AND the Credit & Reminder Settings form. The branch now comes automatically from the user's session scope (`TextileBranchScope::branchIdForCreate()` — assigned branch, header Active Branch dropdown, or employee branch) and is never user-selectable on the page.
  - Filter semantics: parties with **no branch** on their record (`branch_id` null) are global and visible in all branches; parties **with a branch** only show when it matches the session-active branch. This matches prod data (vendors unassigned/global, customers Branch B) — selecting Main Office in the header dropdown now hides Branch B customers.
...
## Inline quotation create/update from Sales screen (deployed 2026-08-11)

- **Commit `8970e3f50`, run 31435204989**: quotations (sauda) can now be created **and** updated directly from `/textile/sales?section=quotations` — no redirect to the standalone `/quotations` page.
- **Create**: the "Create Quotation" button (previously `router.get(route('quotations.create'))`) now opens the `QuotationEditDialog` in create mode — one empty item row, default `quotation_type`, products fetched from `quotations.warehouse.products` (lots for takha/yarn, catalog products for general). Submits to new `POST /textile/sales/quotations` (`textile.sales.quotations.store`) → `TextileSalesController@storeQuotation`.
- **Update**: a new **Actions** column on the quotations table shows an "Edit" button for `draft` quotations only (mirrors Quotation module rule). Opens the same dialog pre-filled with header fields + items (incl. tax rows via `items.taxes`). Submits to `POST /textile/sales/quotations/{quotation}/update` (`textile.sales.quotations.update`) → `TextileSalesController@updateQuotation` (403 unless `created_by` matches, rejects non-draft).
- **Shared dialog**: `Sales/components/QuotationEditDialog.tsx` handles both modes — customer/type/date/due-date/warehouse/payment-terms/notes fields, item table (product select with lot duplicate-lock, qty, unit price, discount %, tax %, per-row totals), live totals summary (subtotal/discount/tax/total), server-side validation errors shown inline.
- **Backend parity**: `storeQuotation`/`updateQuotation` reuse `calculateQuotationTotals()` (mirrors `QuotationController::calculateTotals`: subtotal + tax − discount) and `createQuotationItems()` (recreates item rows + `sales_quotation_item_taxes` rows); update deletes old items first (taxes cascade via FK).
- **Test**: `test_quotations_can_be_created_and_updated_inline_from_sales_screen` covers inline create (totals 10×100 −10% +5% → 1000/100/45/945), inline update of a draft (items replaced, totals recalculated), and rejection of non-draft updates. Full textile suite 96 passed (1643 assertions); `npx tsc --noEmit` 0 errors; `npm run build` passed.
- **Status actions** (commit `f250996a0`, run 31437395602): the quotations Actions column now also shows workflow status buttons — **Send** on `draft` (permission `sent-quotations`), **Approve**/**Reject** on `sent` (`approve-quotations`/`reject-quotations`). They reuse the Quotation module's existing permission-gated routes (`quotations.sent`/`approve`/`reject`) with server-enforced status guards, so the strict draft → sent → accepted/rejected flow is preserved. Status is intentionally NOT a free-form field in the edit dialog — it is workflow-controlled via actions, exactly like the standalone Quotation module.
- **Items detail column** (commit `f0a5cf5c7`, run 31438858372): the quotations table now has an **Items** column showing what each quotation is for — lot references for takha/yarn quotations (with quantity + unit), product names for general quotations (with `qty × price`). Backend `quotations()` now eager-loads `items.product:id,name,sku,unit` and maps `product_name`/`product_sku`/`unit` per item. Verified on prod via grep (mapping + `header: t('Items')` present).
...
## Dispatch form consolidated onto Vehicle entity (deployed 2026-08-11)

- **Commit `f0e44c852`**: removed the redundant **Dispatch Mode**, **Truck Number**, and **Container Number** fields from the Create Dispatch Plan form — they duplicated what the Vehicle entity already captures (`vehicle_type` truck/container, `vehicle_number`, `container_number`), and users were entering two different plates for the same truck (e.g. Truck Number `MH01 AB 1234` vs Vehicle `MH-12-AB-1234`).
- **Server-side derivation** (`TextileDispatchController@storeDispatchPlan`): when `vehicle_id` is provided, `dispatch_mode`/`truck_number`/`container_number` are derived from the vehicle — `vehicle_type === 'container'` → mode `container` + `container_number` (fallback to the vehicle's own number), otherwise mode `truck` + `truck_number = vehicle_number`. No vehicle → mode defaults to `truck`. Fields remain `nullable` in validation for backward compatibility; `TextileDispatchService` also defaults `dispatch_mode` to `truck`.
- **Form**: Vehicle helper text now explains "Truck/container mode and vehicle number are recorded automatically from the selected vehicle." The Mode/Truck/Container columns in the plans tables stay — they show the derived values. Unused page props (`dispatchModeOptions`, `truckNumberOptions`, `containerNumberOptions`) removed from the Inertia share and page; the private controller option methods remain for validation.
- **Tests**: `TextileDispatchAdminTest` updated — truck plan from `vehicleA` derives `dispatch_mode=truck`, `truck_number=GJ01-VH-7788`; container plan from a **distinct second challan** (`CHL-D-003`) with `vehicleB` derives `dispatch_mode=container`, `container_number=MH12-VH-9988` (note: `createDocument` dedupes on source, so two plans cannot share one challan). Redundant `dispatch_mode`/`truck_number` payloads removed from all plans.store posts.
...
## Per-party product pricing — inline on vendor/customer forms (deployed 2026-08-11)

- **Request**: per-party pricing variance (different vendors get different yarn/job-work rates; different customers get different selling rates) should be manageable from ONE place — the vendor/customer form itself, no separate price-entry screen. User: "if i open a vendor i can do all things in one place — credit, per-kg price, whatever."
- **Design** (layered pricing): product master default → per-party price list (this slice) → rate editable per transaction. Different vendors/customers get different prices; the party price list pre-fills and stays editable per order.
- **Data model**: new migration `2026_08_12_000001_create_account_vendor_price_lists_table` — mirror of the existing `account_customer_price_lists`: `vendor_id`, `product_service_item_id`, `unit_price`, `currency_code` (default USD), `min_quantity` (default 1), `is_active`, `notes`, `creator_id`, `created_by`; unique `acc_vendor_prices_unique_idx` on (created_by, vendor_id, product_service_item_id, currency_code). New model `Workdo\Account\Models\VendorPriceList` (+ `active()` scope, `product()`/`vendor()` relations). `Vendor`/`Customer` models gained `priceLists()` hasMany.
- **Backend sync** (`VendorController`/`CustomerController`): `index()` now eager-loads `priceLists` and ships `items` (tenant-scoped active `ProductServiceItem` list with id/name/sku/unit/purchase_price/sale_price) for the form selector. `store()`/`update()` call a new private `syncPriceLists($request, $party)`: upserts rows via `updateOrCreate` on (created_by, party_id, product_service_item_id) — validates `product_service_item_id > 0` and `unit_price >= 0`, resolves the item tenant-scoped (rejects cross-tenant ids), defaults `currency_code` to INR / `min_quantity` to 1; rows omitted from the payload are deleted for that party. Reads `$request->input('price_lists')` directly (no request-file changes needed — validated data is untouched).
- **Frontend**: Vendor Edit and Customer Edit dialogs now have a **Product Pricing** section (bordered box + helper text) listing per-product Rate + Min Qty rows with a controlled product `Select` (fed by `items`), remove (Trash) per row, and an **Add Product Rate** button. Seeded rows render on open; save round-trips the whole `price_lists` array (create/update/delete in one submit).
- **Seeding** (`database/seeders/TextileProductPricingSeeder.php`, run on prod, company 2): 4 units (Kilogram/Meter/Piece/Cone), 9 products (6 yarns: Combed/Carded 40s/30s/20s; 3 grey fabrics 44"/58"/68"), 27 customer prices (3 customers × 9 products, margins +8%/+4%/+12%), 30 vendor prices (5 vendors × 6 yarns, discounts −3%/0%/−6%/−1.5%/−4%), currency INR. Idempotent.
- **Tests**: `tests/Feature/Account/VendorCustomerPriceListTest.php` — 3 cases: vendor update upserts + updates + deletes price rows via `PUT account.vendors.update`; customer update syncs `account_customer_price_lists`; vendor index exposes `items` + `price_lists` props. `php artisan test --filter=VendorCustomerPriceListTest` → 3 passed (27 assertions); existing `VendorSupplierTypeTest`/`CustomerPriceListTest` still pass (5 total, no regressions).
- **Verified on production** (browser, company login): vendor edit dialog for Shree Yarn Traders shows all 6 seeded yarn rates (169.75 for Combed Yarn 40s etc., matching the −3% vendor discount); changed rate 169.75 → 170.00, saved (PUT → 303 → Inertia reload, success toast), re-opened and confirmed 170.00 persisted in DB + UI. Customer edit dialog for Metro Fashions shows all 9 seeded rates (221.40 for Combed Yarn 40s = 205 × 1.08, fabrics 56.16/73.44/90.72 = 52/68/84 × 1.08), matching the +8% margin. Deployed via rsync (PHP controllers/models + 6 TSX/TS files + `public/build`), caches cleared, PHP-FPM restarted, nginx reloaded, login health check 307.

## PO/SO rate prefill from per-party price lists (deployed 2026-08-11, commit `c0d33432b`)

- **Request follow-up**: after per-party pricing shipped, the per-product prices were NOT yet consumed in transaction forms — requisitions used manual `invoice_amount` and sales orders used a single `default_rate`. User asked "how do payments work now" → answer: price data is live, but rates must be prefilled from the party's price list in PO/SO entry forms.
- **Backend**:
  - `TextileProcurementController`: `index()` now ships `items` (tenant-scoped active `ProductServiceItem` with id/name/sku/unit/purchase_price/sale_price) and each vendor in `suppliers` now includes `price_lists` (active `VendorPriceList` entries with product info, mapped to `product_service_item_id/product_name/product_sku/unit/unit_price/min_quantity/currency_code`). `storeRequisition()` accepts `vendor_id` + `product_service_item_id` + `rate`, computes `invoice_amount = round(rate × quantity, 2)`, and stores them (plus `item_name`/`item_sku`) in requisition metadata.
  - `TextileSalesController`: same `items` + per-customer `price_lists` props. `storeSalesOrder()` accepts `product_service_item_id`; when no rate is submitted it resolves from the customer price list (fallback `item.sale_price`), and records `rate_source` (`manual` | `customer_price_list` | `item_sale_price`). A submitted rate is always respected as-is (editable).
- **Frontend**: Requisition form gained a Supplier price-aware Product selector + Rate per Unit field + green Expected Amount preview (`rate × qty`); product options come from the selected supplier's price list (`name (SKU) @ rate`) when available, else all items at purchase price. Sales order form gained the same Product selector prefilling from the customer price list (else sale price / `default_rate`), with helper text guiding rate behaviour. Requisition list now shows Item / Rate / Amount columns from metadata. Shared `SupplierSummary` type extended with optional `price_lists`.
...
- [x] **P-A: Sales invoice from challan** — `TextileSalesService::createSalesInvoiceFromChallan(int $challanId)`: find tenant challan, require status `approved|released|closed`, idempotent via challan metadata `sales_invoice_id`, resolve client user from challan customer, create `SalesInvoice` + `SalesInvoiceItem`(s) (rate from metadata, fallback customer price list → item sale price), store `sales_invoice_id`/`sales_invoice_number` in challan metadata, set `sales_invoice_created_now` attribute. Controller action `TextileSalesController::generateSalesInvoiceFromChallan` + POST route `textile.sales.invoices.generate`. UI: "Generate Invoice" row action on challan records in `Sales/Index.tsx` (challan & challan-pod sections); when invoice exists show its number linking to `sales-invoices.show`. Tests: `tests/Feature/Textile/TextileSalesInvoiceFromChallanTest.php` (happy path amount/rate/customer, idempotency, wrong-status rejection, rate fallback). Verified: 5/5 tests green, full suite 112 passed, deployed + prod invoice visible (TX-CHL-000001).
- [x] **P-B: Purchase invoice line items from GRN/PO** — upgrade `createPurchaseInvoiceFromGrn()` to create `PurchaseInvoiceItem` rows from PO/GRN metadata (`product_service_item_id`, `qty`, `unit`, `rate`, `invoice_amount`). Keep single-total fallback when no item data. Extend `PurchaseInvoice` creation with per-line `total` and keep totals consistent. UI: show line items on the generated invoice (core purchase invoice view already lists items). Tests: `tests/Feature/Textile/TextilePurchaseInvoiceItemsTest.php`. Verified: 2/2 tests green (incl. vendor user id resolution `resolveVendorUserId()` — metadata `vendor_id` is `Vendor.id` while `PurchaseInvoice.vendor_id` is `users.id`), full suite 112 passed, prod invoice #8 backfilled with line item + vendor user 7.
- [x] **P-C: Job-work invoice from yarn dispatch / processing outward** — `TextileSalesService::createJobWorkInvoiceFromDispatchPlan(int $dispatchPlanId, ?float $rate = null)`: find tenant `dispatch_plan`, require status `approved|released|closed`, restrict `metadata.source_type` to `yarn_dispatch|job_work_outward`, idempotent via plan metadata `job_work_invoice_id`. Resolves the sizing/processing vendor (`party_name` → `Workdo\Account\Models\Vendor.company_name`) → its linked client user, else `firstOrCreate` a `type='client'` user (email fallback `jobwork{id}@<host>`) and backfill `vendor.user_id`. Creates/uses a tenant service item ("Job Work - Sizing" for yarn dispatch, "Job Work - Processing" for outward, `type='service'`) and a `SalesInvoice` with `type='service'`, number `TX-JW-%06d`, status `draft`, line item with qty × rate (rate arg, fallback item sale price). Controller action `TextileDispatchController::generateJobWorkInvoice` + POST route `textile.dispatch.job-work-invoices.generate`. UI: "Generate Invoice" row action (with optional rate dialog) on approved/released/closed dispatch plans with `source_type` yarn_dispatch/job_work_outward in `Dispatch/Index.tsx`; when generated, shows `job_work_invoice_number` linking to `sales-invoices.show`. Invoice Hub Job-Work tab picks it up via `type='service'`. Tests: `tests/Feature/Textile/TextileJobWorkInvoiceTest.php` 7/7 green (happy path amount/rate/customer/service item, idempotency, draft rejection, challan-source rejection, job-work-outward processing item, controller success, tenant isolation). Verified: full textile suite 122 passed / 1731 assertions, `npx tsc --noEmit` clean, `npm run build` green.
- [x] **P-D: Textile Invoice Hub page** — a single page under the Textile sidebar (menu + submenu + breadcrumb + page title, per navigation checklist) listing all invoices: tabs for Sales / Purchase / Job Work, each linking to the core invoice show/edit pages, showing status + amounts + party. Ship KPIs (total count, draft, posted, unpaid balance). Reuse `TextileWorkspace` + shared components (`textile-data-table-card`, etc.). Tests: `tests/Feature/Textile/TextileInvoiceHubTest.php`. Verified: 4/4 tests green (tenant scoping, no cross-tenant leak, service-type grouped under job-work, non-textile user 403), full suite 116 passed, `npx tsc --noEmit` clean, `npm run build` green. Implemented: capability `invoices` (sales‖procurement‖payments), workspace `invoices` with Overview/Sales/Purchase/Job Work sections, `TextileInvoiceController` tenant-scoped (sales excludes `type='service'`), route `textile.invoices.index`, sidebar menu entry under Daily Operations.
- [x] **P-D follow-up: Invoice Hub actions + warehouse fix (deployed 2026-08-12)** — (1) All hub tables (Overview/Sales/Purchase/Job Work) now have an Actions column mirroring the core invoice pages: Print (PDF download, `?download=pdf`), View, Post (draft only), Edit (draft only), Delete (draft only, confirmation dialog) — snake_case Ziggy params (`sales_invoice`/`purchase_invoice`), gated by `view/post/print/edit/delete-*-invoices` permissions, delete uses `sales-invoices.destroy`/`purchase-invoices.destroy`. (2) Warehouse crash fix: GRN-generated purchase invoices carried `warehouse_id = null`, and posting crashed with `SQLSTATE 1048 Column 'warehouse_id' cannot be null` on `warehouse_stocks`. Fixed fail-open in `PostPurchaseInvoiceListener`/`PostSalesInvoiceListener` (`resolveDefaultWarehouseId`: active-branch warehouse → first active tenant warehouse → skip stock update), new shared `TextileWarehouseResolver` wired into `TextileProcurementService::createPurchaseInvoiceFromGrn` and `TextileSalesService::createSalesInvoiceFromChallan`, and the requisition form now has a warehouse select (branch-scoped `warehouses` prop from `HasBranchWarehouseScope`) storing `warehouse_id` in metadata (flows requisition → PO → GRN → invoice). Tests: `tests/Feature/Textile/TextilePurchaseInvoiceItemsTest.php` 4/4 green (incl. 2 new warehouse regression tests: explicit `warehouse_id` chains through metadata to the invoice, legacy requisition resolves the branch warehouse at invoice creation), 118 passed / 1862 assertions across textile + account suites, `npx tsc --noEmit` clean, `npm run build` green.

### Acceptance checklist (mandatory before marking any phase `[x]`)
- [x] Backend creates the invoice inside a transaction with tenant scoping (`created_by = creatorId()`) and idempotency guard.
