...
const coreComponents = {
  ...
  'module-governance-settings': lazy(() => import('@/pages/settings/components/module-governance-settings')),
};

// Auto-load package components
const getPackageComponents = (activatedPackages: string[]) => {
  try {
    const modules = import.meta.glob('../../../packages/DigitalFuzed/*/src/Resources/js/settings/components/*.tsx');
    ...
    activatedPackages.forEach(packageName => {
      Object.entries(modules).forEach(([path, moduleLoader]) => {
        if (path.includes(`/packages/DigitalFuzed/${packageName}/`)) {
